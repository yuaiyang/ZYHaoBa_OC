...
- (void)justbegin{
    ...
    CGPoint panPoint = CGPointMake(SCREEN_WIDTH-80, SCREEN_HEIGHT-150);
...
- (void)changBoundsabovePanPoint:(CGPoint)panPoint{
    ...
    {
        if(panPoint.y <= 40+kk_HEIGHT/2 && panPoint.x < SCREEN_WIDTH-kk_WIDTH/2-20 )
        {
            [UIView animateWithDuration:animateDuration animations:^{
                self.center = CGPointMake(panPoint.x, kk_HEIGHT/2);
            }];
